...
- "Escuchar" es una acción explícita por burbuja (T-056): evita sonido inesperado en un dispositivo compartido y gasto innecesario de la API. **Única excepción (T-079)**: si la pregunta se dictó por voz (el texto vino del micrófono), la respuesta de ELI se lee sola una vez, al terminar. `useTutorChat` guarda `speakReplyId` (solo tras un turno terminado con normalidad: nunca si falla o si la niña pulsa «Parar»); `MessageBubble` la reproduce por el camino de siempre (`/api/speech`, con `speechSynthesis` de respaldo). Requiere el flag `voice_mode` (`allowVoice`). Al activar el micrófono o enviar se corta cualquier voz en curso (`stopAllSpeech`), para que ELI no se oiga a sí misma como si fuera la pregunta. Escribir una pregunta nueva desde cero vuelve al comportamiento manual. Los navegadores que bloqueen el autoplay caen a la voz del navegador y, si tampoco, queda el botón «Escuchar».
...
- **Interfaz (T-083): tres secciones, cada una en su ruta.** `/admin` redirige a `/admin/users`; `/admin/users` (cuentas, roles y flags por cuenta), `/admin/config` (proveedor, modelos, voz y límites) y `/admin/features` (feature flags globales). `app/(app)/admin/layout.tsx` decide el acceso **en el servidor** con `getAdminEmail()` (`lib/auth/adminGuard.ts`): sin admin autenticado responde 404 y ni se envía la interfaz (antes se enviaba y era el cliente quien mostraba «Página no encontrada»). La navegación (`AdminNav`) marca la sección activa con `useSelectedLayoutSegment`. La carga de `GET /api/admin/config` se comparte en `useAdminConfig` (`components/admin/shared.tsx`).
