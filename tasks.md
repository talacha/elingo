...
| N-010 | Aviso de 429 con cuenta atrás: desactivar «Reintentar» hasta que pase `retryAfter` (hoy el botón está siempre activo) | T-014 |
| N-011 | `next dev` (Next 16) añade un bloque `nextjs-agent-rules` a `CLAUDE.md` en cada arranque; decidir si se commitea una vez o se evita, para que los agentes no arrastren ese cambio | T-014 |
