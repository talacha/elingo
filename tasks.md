...
| N-010 | Los PRs #5 y #6 se fusionaron por auto-merge con solo el commit `in progress` (un único archivo, `tasks.md`) y `--delete-branch` borró la rama-lock con la tarea aún en curso (T-020 tuvo que reabrir el PR #10). Propuesta: en el paso 3 solo se publica la rama; `gh pr create` y `gh pr merge --auto` van exclusivamente en el paso 6, y `scripts/tasks-check.mjs` podría avisar si un PR abierto solo toca `tasks.md` | T-011 |
